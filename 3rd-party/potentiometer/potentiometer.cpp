...
{
    Q_INIT_RESOURCE(potentiometer);
    m_renderer = NULL;
    m_rotationRange = 100;
    m_value = value() - minimum();
    setRange(0, 100);
    setSvgResourceFile(":/potentiometer/dial2.svg");
}
...
{ 
    ...
    // Rysowanie Kropki (Polish Draw Point)
    ...
    painter.save();
    //painter.rotate(paintValue * step_angle + 180);
...
void PotentioMeter::mouseMoveEvent(QMouseEvent* event)
{
    if(event->buttons() & Qt::LeftButton)
    {
        ...
        //QLineF line(width() / 2, height() / 2, width() / 2, height());
        //QLineF line2(width() / 2, height() / 2, 0, height());
        ...
        if(value() % rRange - m_inc != 0 && m_inc != 0)
        {
            ...
            //qDebug("m_inc %d, v %d", m_inc, v);
            //qDebug("angle %f, angle2 %f", angle, angle2);
            ...
            //qDebug("diff %d, index %d", diff, index);
            //qDebug("value %d, m_value %d", value(), m_value);
            ...
            //qDebug("angle %f, m_prevAngle %f", angle, m_prevAngle);
            if((angle <= 90 && m_prevAngle >= 270) ||
                (angle >= 90 && m_prevAngle <= 90) ||
                (angle >= 180 && m_prevAngle <= 180) ||
                (angle >= 270 && m_prevAngle <= 270))
            //if(-1 == v)
            {
                emit valueInc();
            }

            if((m_prevAngle <= 90 && angle >= 270) ||
                (m_prevAngle >= 90 && angle <= 90) ||
                (m_prevAngle >= 180 && angle <= 180) ||
                (m_prevAngle >= 270 && angle <= 270))
            //if(1 == v)
            {
                emit valueDec();
            }
